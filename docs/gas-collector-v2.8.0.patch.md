# PinArchive Google Apps Script (GAS) Collector v2.8.0 Patch Document

This document contains the canonical Google Apps Script (GAS) Collector v2.8.0 source code for PinArchive, providing high-performance batch updates, resilient ScriptLock handling, and the new batch `account_ages` endpoint for extracting the oldest known pin timestamp per creator tab from Google Sheets.
...
> 4. Replace `Code.gs` contents with the complete `gas-collector-v2.8.0.gs` code below.
> 5. Click **Save** (`Ctrl+S`).
> 6. Click **Deploy > Manage deployments > Edit > New version > Deploy**.
> 7. **Canary Validation Gate**:
>    - Send a test ping payload: verify response returns `{ ok: true, version: "2.8.0", ... }`.
>    - Test action `account_ages` with `usernames: ["tehifat570"]`: verify response returns `{ ok: true, version: "2.8.0", ages: { "tehifat570": "2026-04-09T00:14:38.000Z" } }`.

---

## 1. Changelog (v2.8.0 vs v2.7.1)

- **[F1/F2] Added Batched `account_ages` Action**:
  - Adds dedicated `account_ages` action to `doPost` to calculate the oldest known pin timestamp per account directly from its Google Sheet tab (`pins_<username>`).
  - Scans only the `created_at` column via `schema.map` range extraction (`sh.getRange(2, colIdx, lastRow - 1, 1).getValues()`) for minimal I/O overhead.
  - Computes `min(created_at)` across all rows without assuming chronological row order.
  - Handles both native `Date` objects and string formatted timestamps (`Date.parse()`).
  - Capped at 50 usernames per batch call. Never throws per username: missing sheet or unparseable dates safely return `null`.
- **[Telemetry] Version Bump**:
  - Bumped collector version to `2.8.0` across `doGet`, `ping`, and `account_ages`.

---

## 2. Deployed Source Code (`gas-collector-v2.8.0.gs`)

```javascript
/***************************************************************
 * 📌 PinArchive Collector v2.8.0 — HIGH-PERFORMANCE THIN WRITER
 *
 * Changelog vs v2.7.1:
...
function doPost(e) {
  ...
  switch (p.action) {
    case 'ping':
      return out_({ok: true, version: '2.8.0', legacy_mode: isLegacyMode_()});
    case 'status':
...
function handleAccountAges_(p) {
  const usernames = Array.isArray(p.usernames)
    ? p.usernames.map(String).map(s => s.trim()).filter(Boolean).slice(0, 50)
    : [];
  ...
  for (const username of usernames) {
    ...
    try {
      ...
      const colIdx = schema.map['created_at'];
      if (!colIdx) continue;
  ...
  return out_({ ok: true, version: '2.8.0', ages: ages });
}
...
function handleSheetWrite_(p) {
  ...
  if (rawRows.length === 0) return out_({ok: true, version: '2.8.0', total_received: 0, written: 0, appended: 0, updated: 0, unchanged: 0});
