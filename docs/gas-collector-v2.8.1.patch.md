# PinArchive Google Apps Script (GAS) Collector v2.8.1 Patch Document

This document contains the canonical Google Apps Script (GAS) Collector v2.8.1 source code for PinArchive, providing high-performance batch updates, resilient ScriptLock handling, and the new batch `account_ages` endpoint for extracting the oldest known pin timestamp per creator tab from Google Sheets.
...
> 4. Replace `Code.gs` contents with the complete `gas-collector-v2.8.1.gs` code below.
> 5. Click **Save** (`Ctrl+S`).
> 6. Click **Deploy > Manage deployments > Edit > New version > Deploy**.
> 7. **Canary Validation Gate**:
>    - Send a test ping payload: verify response returns `{ ok: true, version: "2.8.1", ... }`.
>    - Test action `account_ages` with `usernames: ["tehifat570"]`: verify response returns `{ ok: true, version: "2.8.1", ages: { "tehifat570": "2026-04-09T00:14:38.000Z" } }`.

---

## 1. Changelog (v2.8.1 vs v2.8.0)
...
## 2. Deployed Source Code (`gas-collector-v2.8.1.gs`)

```javascript
/***************************************************************
 * 📌 PinArchive Collector v2.8.1 — HIGH-PERFORMANCE THIN WRITER
 *
 * Changelog vs v2.7.1:
...
function doPost(e) {
  ...
  switch (p.action) {
    case 'ping':
      return out_({ok: true, version: '2.8.1', legacy_mode: isLegacyMode_()});
    case 'status':
...
function handleAccountAges_(p) {
  ...
  return out_({ ok: true, version: '2.8.1', ages: ages });
}
...
function handleSheetWrite_(p) {
  ...
  if (rawRows.length === 0) return out_({ok: true, version: '2.8.1', total_received: 0, written: 0, appended: 0, updated: 0, unchanged: 0});
