# PinArchive Google Apps Script (GAS) Collector v2.8.3 Patch Document

This document contains the canonical Google Apps Script (GAS) Collector v2.8.3 source code for PinArchive, providing resilient ingest contract safety, 409 account-skip non-terminal handling, body `json.skipped` verification, and immutable version co-existence guarantees.
...
> 4. Replace `Code.gs` contents with the complete `gas-collector-v2.8.3.gs` code below.
> 5. Click **Save** (`Ctrl+S`).
> 6. Click **Deploy > Manage deployments > Edit > New version > Deploy**.
> 7. **Canary Validation Gate**:
>    - Send a test ping payload: verify response returns `{ ok: true, version: "2.8.3", ... }`.
>    - Test action `account_ages` with `usernames: ["tehifat570"]`: verify response returns `{ ok: true, version: "2.8.3", ages: { "tehifat570": "2026-04-09T00:14:38.000Z" } }`.

---

## 1. Immutable Version Co-Existence Plan (v2.8.3 & v2.8.2)

Google Apps Script version deployments are immutable version snapshots (no automated rollback). v2.8.3 is architected for safe side-by-side co-existence with existing v2.8.2 deployments:

1. **409 Account-Skip Non-Terminal Handling**:
   - In PinArchive P4, account-level skips (e.g. `account_ingest_disabled`, `account_paused`) return HTTP 409 `{ success: false, error: 'account_*', skipped: 'account_*', retryable: false }`.
   - **v2.8.3 behavior**: Recognizes `code === 409 && (error === 'ingest_disabled' || error.indexOf('account_') === 0)` as `{ ok: true, skipped: error }`. It gracefully skips the account without logging fatal errors and without marking rows with `archived_at` stamps.
   - **v2.8.2 behavior**: Treats HTTP 409 as `{ ok: false, error: ... }`. It skips `archived_at` stamping and logs the response. No data is lost, and no false stamps are written.
2. **2xx Body Verification**:
   - On HTTP 200/202 responses, v2.8.3 parses `json.skipped`. If `skipped` is present, it returns `{ ok: true, skipped: json.skipped }`, ensuring rows are never stamped as archived when the server skipped writing them.
3. **Rollout Runbook**:
   - Deploy v2.8.3 to a single canary spreadsheet first.
   - Validate live ingest execution on active and paused accounts.
   - Once verified, promote v2.8.3 to remaining production spreadsheets.

---

## 2. Changelog (v2.8.3 vs v2.8.2)
...
## 3. Deployed Source Code (`gas-collector-v2.8.3.gs`)

```javascript
/***************************************************************
 * 📌 PinArchive Collector v2.8.3 — HIGH-PERFORMANCE THIN WRITER
 *
 * Changelog vs v2.8.2:
...
function readHeaderMap_(sh) {
  const w0 = sh.getLastColumn();
  if (w0 <= 0) return {};
  const row1 = sh.getRange(1, 1, 1, w0).getValues()[0];
  const map = {};
...
function parseSheetTimestampMs_(cell) {
...
function rowNeedsUpdate_(existRow, map, r) {
  const fields = ['title','description','link','domain','board_name','created_at',
    'image_url','image_signature','dominant_color','saves','repins','comments','velocity','tags'];
  for (const f of fields) {
    const oldVal = String(getF_(existRow, map, f) ?? '').trim();
    let newVal = r[f];
    if (f === 'created_at' && newVal === undefined) newVal = r.created_at_pinterest;
    if (Array.isArray(newVal)) newVal = newVal.join(', ');
    const newStr = String(newVal ?? '').trim();
    if (newStr && newStr !== oldVal) return true;
  }
  return false;
}
...
function doPost(e) {
  let b = {};
  try { b = JSON.parse((e && e.postData && e.postData.contents) || '{}'); } catch (err) { return out_({ok: false, error: 'bad json'}); }
  ...
  // ── Action: account_ages (Batch Oldest Pin Timestamps for PinArchive UI) ──
  if (p.action === 'account_ages') {
    return handleAccountAges_(p);
  }
  ...
  switch (p.action) {
    case 'ping':
      return out_({ok: true, version: '2.8.3', legacy_mode: isLegacyMode_()});
    case 'status':
...
/* ================= معالج حساب أقدم بين لكل حساب (account_ages) ================= */
function handleAccountAges_(p) {
  const USERNAME_RE = /^[A-Za-z0-9._-]{1,64}$/;
  const usernames = Array.isArray(p.usernames)
    ? p.usernames.map(String).map(s => s.trim()).filter(s => USERNAME_RE.test(s)).slice(0, 50)
    : [];
  ...
  for (const username of usernames) {
    ...
    try {
      ...
      const headerMap = readHeaderMap_(sh);
      const colIdx = headerMap['created_at'];
      if (!colIdx) continue;

      // Read only the created_at column across all data rows (1 column range)
      const rawValues = sh.getRange(2, colIdx, lastRow - 1, 1).getValues();
      let minMs = Infinity;
      let minIso = null;

      for (let i = 0; i < rawValues.length; i++) {
        const cell = rawValues[i][0];
        if (cell === '' || cell === null || cell === undefined) continue;
        const t = parseSheetTimestampMs_(cell);
  ...
  return out_({ ok: true, version: '2.8.3', ages: ages });
}
  ...
  try {
    ...
    for (const username of usernames) {
      ...
      for (let i = 0; i < rawValues.length; i++) {
        ...
        const createdMs = parseSheetTimestampMs_(getF_(row, map, 'created_at'));
        const ageDays = Number.isFinite(createdMs) ? Math.max(0, (Date.now() - createdMs) / 86400000) : 99999;
        ...
        const pinObj = {
          ...
          created_at: cellToStr_(getF_(row, map, 'created_at')),
          created_at_pinterest: cellToStr_(getF_(row, map, 'created_at')),
          saves: saves,
          repins: repins,
          comments: comments,
          age_days: ageDays,
      ...
      let accountSynced = 0;
      const rowIndicesSynced = [];
      if (qualifyingPins.length > 0) {
        for (let b = 0; b < qualifyingPins.length; b += 500) {
          const batch = qualifyingPins.slice(b, b + 500);
          const batchRows = rowIndicesToStamp.slice(b, b + 500);
          const sendRes = sendToPinOrbit_({ workspace_id: wsId, username: username }, batch, {
            pins_count: rawValues.length,
            last_result: 'sheet_sync'
          });
          if (sendRes.ok && !sendRes.skipped) {
            accountSynced += batch.length;
            for (const rIdx of batchRows) rowIndicesSynced.push(rIdx);
          } else {
            console.error('sheet_sync failed/skipped for @' + username + ': ' + (sendRes.skipped || sendRes.error || 'unknown'));
          }
        }

        if (map['archived_at'] && rowIndicesSynced.length > 0) {
          const colIdx = map['archived_at'];
          for (const rIdx of rowIndicesSynced) {
...
function handleSheetWrite_(p) {
  ...
  if (rawRows.length === 0) return out_({ok: true, version: '2.8.3', total_received: 0, written: 0, appended: 0, updated: 0, unchanged: 0});
...
function sendToPinOrbit_(acc, pins, accountMeta) {
  ...
  let error = '';
  let bodyJson = null;
  try { bodyJson = JSON.parse(res.getContentText()) || null; } catch (e) {}

  if (!(code >= 200 && code < 300)) {
    error = (bodyJson && bodyJson.error) ? bodyJson.error : ('http ' + code);
  }

  // Handle both workspace-level (ingest_disabled) and account-level skips (account_*)
  if (code === 409 && (error === 'ingest_disabled' || error.indexOf('account_') === 0)) {
    return {ok: true, skipped: error};
  }

  if (code >= 200 && code < 300) {
    if (bodyJson && bodyJson.skipped) {
      return {ok: true, skipped: bodyJson.skipped};
    }
    return {ok: true, code};
  }
  ...
  try {
    ...
    for (const item of accounts) {
      ...
      for (let i = 0; i < rows.length; i++) {
        ...
        const repins = Number(getF_(row, map, 'repins')) || 0;
        const createdMs = parseSheetTimestampMs_(getF_(row, map, 'created_at'));
        const age = Number.isFinite(createdMs) ? Math.max(1, (Date.now() - createdMs) / 86400000) : 99999;
...
function readAccounts_() {
  ...
  for (let r = 1; r < v.length; r++) {
    ...
    list.push({
      row: r + 1,
      acc: {
        username, user_id: String(row[C_.user_id - 1] || ''),
        workspace_id: String(row[C_.workspace_id - 1] || '').trim(),
        sheet_name: String(row[C_.sheet_name - 1] || ('pins_' + username)).trim(),
