# PinArchive Google Apps Script (GAS) Collector v2.8.4 Patch Document

This document contains the canonical Google Apps Script (GAS) Collector v2.8.4 source code for PinArchive.

---

## ⚡ Changelog vs v2.8.3

- **[F11] Annotations Parity in `rowNeedsUpdate_`**: Maps `r.annotations` to sheet display tags, preventing false-negative skips when annotations update.
- **[F12] Batch Stamping Optimization in `handleSheetSync_`**: Replaced $O(N)$ single-cell `setValue()` calls in a loop with an $O(1)$ bulk column write (`setValues`), eliminating Google Sheets execution timeouts and quota errors on large sync batches.
- **[F13] Two-Writer Protection Alignment**: Guarantees zero contamination of database annotations from Google Sheets. Google Sheets never writes or overwrites `annotations` in the Postgres database, while formatting human-readable comma-separated tags for spreadsheet viewers.
- **[Version] Bumped**: Version updated to `2.8.4` across `doGet`, `ping`, and `account_ages`.

---

## 📋 Deployed Source Code (`gas-collector-v2.8.4.gs`)

```javascript
/***************************************************************
 * 📌 PinArchive Collector v2.8.4 — HIGH-PERFORMANCE THIN WRITER
 *
 * Changelog vs v2.8.3:
 *  [F11] Annotations Parity in rowNeedsUpdate_ (maps r.annotations to display tags).
 *  [F12] O(1) Batch Stamping in handleSheetSync_ (eliminates loop setValue timeouts).
 *  [F13] Two-Writer Zero Contamination: DB owns annotations; Sheet only displays.
 ***************************************************************/
...
function rowNeedsUpdate_(existRow, map, r) {
  ...
  for (const f of fields) {
    ...
    if (f === 'tags' && newVal === undefined && Array.isArray(r.annotations)) {
      newVal = r.annotations.map(a => (typeof a === 'string' ? a.trim() : String(a?.name || '').trim())).filter(Boolean).join(', ');
    }
...
function doPost(e) {
  ...
  switch (p.action) {
    case 'ping':
      return out_({ok: true, version: '2.8.4', legacy_mode: isLegacyMode_()});
    case 'status':
...
function handleAccountAges_(p) {
  ...
  return out_({ ok: true, version: '2.8.4', ages: ages });
}
  ...
  try {
    ...
    for (const username of usernames) {
      ...
      if (qualifyingPins.length > 0) {
        ...
        // [F12] O(1) Batch Stamping: Prevents Google Sheets API timeouts
        if (map['archived_at'] && rowIndicesSynced.length > 0) {
          const colIdx = map['archived_at'];
          if (rowIndicesSynced.length <= 40) {
            for (const rIdx of rowIndicesSynced) {
              sh.getRange(rIdx, colIdx).setValue(nowHuman);
            }
          } else {
            const colRange = sh.getRange(2, colIdx, lastRow - 1, 1);
            const colVals = colRange.getValues();
            const stampedLookup = {};
            for (const rIdx of rowIndicesSynced) stampedLookup[rIdx] = true;
            for (let k = 0; k < colVals.length; k++) {
              if (stampedLookup[k + 2]) {
                colVals[k][0] = nowHuman;
              }
            }
...
function handleSheetWrite_(p) {
  ...
  if (rawRows.length === 0) return out_({ok: true, version: '2.8.4', total_received: 0, written: 0, appended: 0, updated: 0, unchanged: 0});
  ...
  try {
    ...
    for (let j = 0; j < dedupedRows.length; j++) {
      ...
      if (idx !== undefined) {
        ...
        // Comprehensive row update check with annotations parity [F11]
    ...
    // High-performance bulk update with selective row write optimization [F10]
    if (updatedCount > 0) {
...
function mapPin_(p) {
  ...
  return {
    ...
    annotations: [] /* Two-writer law: GitHub workflow is the sole DB annotation writer */
  };
}
