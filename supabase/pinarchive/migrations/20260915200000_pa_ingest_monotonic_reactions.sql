-- Migration: Update pa_ingest_pin_batch with monotonic cumulative reactions protection
-- 1. Enforce monotonic GREATEST preservation on pa_pins.reactions to prevent 0-overwrites from partial SSR responses
-- 2. Guard pa_pin_metrics snapshot reactions_total using GREATEST against existing historical max
-- 3. Include reactions_total in pa_pin_metrics advancement predicate (pm.reactions_total >= u.reactions->>'total')
  ...
  inserted_metrics AS (
    ...
    SELECT
      ...
      GREATEST(
        ...
        coalesce((SELECT MAX(pm_prev.reactions_total) FROM public.pa_pin_metrics pm_prev WHERE pm_prev.pin_ref = u.id), 0)
      )
    FROM upserted u
    WHERE NOT EXISTS (
