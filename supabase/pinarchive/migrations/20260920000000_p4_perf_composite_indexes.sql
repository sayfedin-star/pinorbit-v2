-- ==============================================================================
-- Migration: 20260920000000_p4_perf_composite_indexes.sql
-- Project 4: PinArchive (kuuugffvyokywtgmdrfk)
-- Domain: Unindexed FKs, Composite Cursor Index, Search Path & Internal Triggers Security
-- ==============================================================================

-- 1. Foreign Key Covering Indexes (Clears 0001_unindexed_foreign_keys)
CREATE INDEX IF NOT EXISTS idx_pa_runs_account_id 
  ON public.pa_runs (account_id);

CREATE INDEX IF NOT EXISTS idx_pa_staged_pins_pa_pin_id 
  ON public.pa_staged_pins (pa_pin_id);

-- 2. Composite Cursor Index for Account-Filtered Dispatches
CREATE INDEX IF NOT EXISTS idx_pa_pin_dispatches_ws_acc_sent 
  ON public.pa_pin_dispatches (workspace_id, target_account_id, sent_at DESC, id DESC);

-- 3. Fix 0011_function_search_path_mutable on public.pa_account_pins_page
CREATE OR REPLACE FUNCTION public.pa_account_pins_page(
  ...
  p_sort text DEFAULT 'saves'::text, 
  p_asc boolean DEFAULT false, 
  p_limit integer DEFAULT 50, 
  p_offset integer DEFAULT 0, 
  p_max_saves bigint DEFAULT NULL::bigint, 
  p_min_saves bigint DEFAULT NULL::bigint
)
RETURNS TABLE(
  id uuid, 
  pin_id text, 
...
-- 4. Revoke execute on internal trigger/stats functions (Clears 0028 & 0029 lints)
REVOKE EXECUTE ON FUNCTION public.pa_reconcile_account_stats(p_account_id uuid) FROM public, anon, authenticated;
REVOKE EXECUTE ON FUNCTION public.trg_pa_accounts_init_stats() FROM public, anon, authenticated;
REVOKE EXECUTE ON FUNCTION public.trg_pa_pins_delta_stats() FROM public, anon, authenticated;
