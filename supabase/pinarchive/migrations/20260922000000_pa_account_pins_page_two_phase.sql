-- Migration: 20260922000000_pa_account_pins_page_two_phase.sql
-- Fix [F-04 - P0]: Two-Phase Paging in pa_account_pins_page to avoid calculating lateral metrics for all account pins before LIMIT.
-- Fix [F-05]: Optimize pa_ingest_pin_batch to avoid 1,000 correlated subqueries per batch and check only latest snapshot.

-- 1. pa_account_pins_page
...
BEGIN
  ...
  v_needs_delta_stage := upper(trim(coalesce(p_stage, ''))) IN ('COOLING', 'NEW', 'DORMANT');
  ...
  ELSE
    -- FALLBACK PATH: for sorting by delta_saves/repins/3d/7d or filtering by COOLING/NEW/DORMANT stage
    RETURN QUERY
    WITH base_pins AS (
      SELECT
        p.id,
