-- Migration: Update pa_ingest_pin_batch accounting
-- 1. Accurately calculate v_added_count from deduplicated temp table (v_unique_incoming_count - v_updated_count)
-- 2. Include comments in pa_pin_metrics snapshot predicate (pm.comments >= u.comments)

CREATE OR REPLACE FUNCTION public.pa_ingest_pin_batch(p_workspace_id uuid, p_account_id uuid, p_fetched_at timestamp with time zone, p_pins jsonb, p_dry_run boolean DEFAULT false)
 RETURNS jsonb
 LANGUAGE plpgsql
 SET search_path TO ''
AS $function$
