-- Migration: 20260922000001_pa_pins_updated_idx_and_stage_fix.sql
-- Description: Adds idx_pa_pins_ws_acc_updated, aligns pa_account_pins_page stage filter, and adds deterministic order to pa_ingest_pin_batch to prevent deadlocks

-- 1. B-tree index on pa_pins (workspace_id, account_id, last_updated_at ASC) for fast refresh pagination
CREATE INDEX IF NOT EXISTS idx_pa_pins_ws_acc_updated
ON public.pa_pins USING btree (workspace_id, account_id, last_updated_at ASC);

-- 2. Update pa_account_pins_page: Align fast-path stage filtering with pin-stages.ts and enable fast path for NEW and DORMANT
...
BEGIN
  ...
  IF NOT v_is_delta_sort AND NOT v_needs_delta_stage THEN
    ...
    -- Phase 2: Compute lateral metrics ONLY for the 50 paged pins
    SELECT
      pp.id,
      pp.pin_id,
      pp.title,
  ...
  ELSE
    -- FALLBACK PATH: for sorting by delta_saves/repins/3d/7d or filtering by COOLING stage
    RETURN QUERY
    WITH base_pins AS (
      SELECT
        p.id,
...
-- 3. Update pa_ingest_pin_batch: Add ORDER BY tip.pin_id ASC to ensure deterministic row-lock order and eliminate deadlocks (40P01)
...
BEGIN
  ...
  SELECT count(*)::int INTO v_unique_incoming_count
  FROM temp_incoming_pins;
  ...
  WITH upserted AS (
    ...
    ON CONFLICT (workspace_id, pin_id) DO UPDATE SET
      ...
      comments = GREATEST(excluded.comments, pa_pins.comments),
