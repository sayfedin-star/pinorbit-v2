...
export const POST: APIRoute = async ({ request, locals }) => {
  ...
  // 3. Authenticate via x-ingest-secret against workspace secret
  ...
  // 4. Defensive Cleanup Guard: if job is already terminated, ensure FastCron job is deleted
  ...
  // 5. Concurrency Guard: Atomically claim tick
  ...
  // 6. Fetch connection details and resolve webhook
  ...
  const webhookUrl = claimedJob.channel === 'top_pins'
  ...
  // 7. Dispatch single day to Make.com
  ...
  // 8. Compute next day using UTC midday math
  ...
  // 10. If finished, auto-destruct the FastCron recurring job
