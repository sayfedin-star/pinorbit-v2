...
22. **Webhook Quota Push-vs-Reservation Ordering Trade-off:**
    - *Context & Trade-off:* In `daily-dispatch.ts`, Make push occurs prior to calling `increment_webhook_execution`. While reversing this to reserve quota before push would prevent over-delivery when concurrent dispatches run at near-zero quota capacity, an external push failure would then require complex compensation logic (rollback/refund) across independent network boundaries. Because the dispatch endpoint already checks `remaining_capacity` via initial read and `increment_webhook_execution` enforces the atomic reservation predicate in Postgres, this ordering is an accepted trade-off that prioritizes never dropping dispatched pins over strict pre-push quota blocking.
23. **Ingest Concurrency CAS Guard & Terminal State Safety:**
    - *Atomic Guard:* Webhook callbacks for `pin.posted` apply an atomic CAS update (`.neq('status', 'posted').select('id')`). If zero rows are modified, the handler returns HTTP 200 `{ success: true, handled: 'pin_posted_duplicate' }` and short-circuits without creating duplicate delivery log records or mutating `posted_at`.
    - *Terminal Safety:* Webhook callbacks for `pin.failed` check existing pin status and ignore failure signals if the pin is already in terminal `posted` state (`handled: 'pin_failed_ignored_already_posted'`), protecting against out-of-order network arrival and preventing successful pin deliveries from being erroneously marked as failed.
24. **Explicit Query Bounds & Truncation Signaling:**
    - *Boundary Signaling:* Aggregated list endpoints (`/api/admin/competitors.ts` and `/api/links/index.ts`) provide explicit boundary flags (`truncated: boolean` and `domains_truncated: boolean`) derived from `data.length >= 1000` alongside `.range(0, 999)`.
    - *Defensive Chaining:* In query builders where mocks or test runners may omit chainable `.range()` support, safe conditional invocation (`typeof query.range === 'function' ? query.range(0, 999) : query`) guarantees both test runner compatibility and production boundary safety.
25. **Uniform IANA Timezone Validation & Overview UTC Display:**
    - *Validation Boundary:* All competitor and pinarchive schedule/cron mutation endpoints (`/api/competitors/schedules/index.ts`, `/api/competitors/schedules/[id].ts`, `/api/competitors/cron.ts`, `/api/pinarchive/cron.ts`) share centralized validation in `src/server/lib/timezone.ts` via `isValidTimeZone` using `Intl.DateTimeFormat`. Non-empty invalid timezones are rejected with HTTP 400 `'Invalid timezone. Must be a valid IANA timezone name.'`.
    - *Accepted Overview UTC Display:* In PinArchive (`src/pages/pinarchive/overview.astro`), `pa_workspace_settings` has no `timezone` column in the live database schema. Next-run calculations deliberately default to `'UTC'`, avoiding dead-code schema assumptions while maintaining accurate deterministic calculation across the dashboard.
